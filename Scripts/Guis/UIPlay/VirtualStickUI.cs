...
public class VirtualStickUI : MonoBehaviour 
{
	...
	{ 
		...
		//remark by sidney 测试用
		//Transform target = JxBlGame.Instance.transform; 
		Transform target = GameObject.Find ("JxBlGame").transform;
		...
		//remark by sidney 测试用,其实这句写不写都无所谓，要是没有状态，很多功能都没有用，也不在乎这个stickui功能了，所以player必须要有状态
        //if (playState == null)
        //    return;
        ...
        //获取当前相机类型
		if (playState==null || playState.mCameraType == 1)         //斜45度
        {
            //转为相机坐标系
